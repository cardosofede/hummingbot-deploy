...
class LupinV2Config(ControllerConfigBase):
    """
    Configuration required to run the D-Man Maker V2 strategy.
    """
    controller_name: str = "lupin_v2"
    ...
    min_spread_between_executors: Decimal = Decimal("0.05")
    ...
    take_profit: Decimal = Decimal("0.005")
    time_limit: int = 86400
    trailing_stop: TrailingStop = TrailingStop(
        activation_price=Decimal("0.0045"),
...
class LupinV2(ControllerBase):
    def __init__(self, config: LupinV2Config, *args, **kwargs):
        self.config = config
            ...
            if len(processed_data["active_order_placed"]) == 0:
                if processed_data["size"] == Decimal("0"):
                    ...
                    actions.append(action)
                        ...
                        processed_data["distance_to_closest"] > self.config.min_spread_between_executors:
                    action = self.create_executor_action(side)
                    actions.append(action)
        return actions
...
Lupin Config:
    ...
    Trailing stop: {self.config.trailing_stop}
""")
        for side in [TradeType.BUY, TradeType.SELL]:
